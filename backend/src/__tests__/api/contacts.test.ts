...
import * as contactsService from '@/db/contactsService'

jest.mock('@/db/contactsService')
...
describe('GET /api/contacts', () => {
  ...
  it('should return 200 with contacts array', async () => {
    const mockContacts = [
      { id: 1, type: 'Email', info: 'test@example.com', icon: 'mail' },
      { id: 2, type: 'Phone', info: '+1 234 567 890', icon: 'phone' },
    ]
  ...
  it('should handle contacts with null icon', async () => {
    const mockContacts = [{ id: 1, type: 'Email', info: 'test@example.com', icon: null }]
