 ...
 * Revalidates the adminPage's Next.js cache so it reflects changes in real time.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expectedSecret) {
      console.warn('[Revalidate Admin] REVALIDATION_SECRET not configured, skipping check')
    ...
    console.log(`[Revalidate Admin] Revalidating tag: "${tag}"`)
    revalidateTag(tag)

    return NextResponse.json({ revalidated: true, tag })
  } catch (error) {
    console.error('[Revalidate Admin] Error:', error)
    return NextResponse.json(
